...
async def cb_overbought(query: CallbackQuery) -> None:
    ...
    msg = await query.message.answer("Ищу перекупленные монеты...")
    try:
        ...
        await msg.edit_text(text or "Ничего не найдено.", parse_mode="Markdown", reply_markup=back_keyboard())
...
async def cb_social(query: CallbackQuery) -> None:
    ...
    msg = await query.message.answer("🔍 Анализирую Reddit, CoinGecko, Dex...")
    try:
...
async def free_symbol(message: Message) -> None:
    ...
    wait = await message.answer(f"📊 Анализирую {sym}...")
    try:
        await wait.edit_text(await analyzer.full_analysis(sym), parse_mode="Markdown")
    except Exception as e:
        await wait.edit_text(f"❌ Ошибка: {str(e)[:120]}")
...
async def cb_gems(query: CallbackQuery) -> None:
    ...
    msg = await query.message.answer("Сканирую рынок...")
    try:
        text = await analyzer.find_new_potential()
        await msg.edit_text(text, parse_mode="Markdown", reply_markup=back_keyboard())
    except Exception as e:
        await msg.edit_text(f"❌ Ошибка: {str(e)[:120]}", reply_markup=back_keyboard())
...
async def cb_alerts(query: CallbackQuery) -> None:
    save_user_id(query.from_user.id)
    await query.answer()
    await query.message.answer("🔔 Функция алертов активна. Скоро добавлю UI управления.", reply_markup=back_keyboard())


@dp.message(Command("help"))
async def help_cmd(message: Message) -> None:
    save_user_id(message.from_user.id)
    await message.answer("Используй кнопки меню или команды /analyze /gems /social /scam /signals /sniper")
...
async def gems_cmd(message: Message) -> None:
    save_user_id(message.from_user.id)
    m = await message.answer("💎 Ищу gem'ы...")
    try:
        await m.edit_text(await analyzer.find_new_potential(), parse_mode="Markdown", reply_markup=back_keyboard())
    except Exception as e:
        await m.edit_text(f"❌ Ошибка: {str(e)[:120]}")


@dp.message(Command("social"))
async def social_cmd(message: Message) -> None:
    save_user_id(message.from_user.id)
    m = await message.answer("📡 Сканирую соцсигналы...")
    try:
        await m.edit_text(await social_scanner.find_social_gems(), parse_mode="Markdown", reply_markup=back_keyboard())
    except Exception as e:
        await m.edit_text(f"❌ Ошибка: {str(e)[:120]}")
...
async def scam_cmd(message: Message) -> None:
    save_user_id(message.from_user.id)
    m = await message.answer("🚨 Сканирую скам...")
    try:
        await m.edit_text(await analyzer.find_scam_whales(), parse_mode="Markdown", reply_markup=back_keyboard())
    except Exception as e:
        await m.edit_text(f"❌ Ошибка: {str(e)[:120]}")
...
async def signals_cmd(message: Message) -> None:
    save_user_id(message.from_user.id)
    m = await message.answer("📈 Ищу топ сигналы...")
    try:
        gems = await analyzer.find_gems()
        text = "📈 *Топ сигналы:*\n\n" + "\n".join(
            f"{i+1}. {g.get('symbol','').upper()} — 24ч {float(g.get('price_change_percentage_24h') or 0):+.2f}%"
...
@dp.message(Command("sniper"))
async def sniper_status_cmd(message: Message) -> None:
    save_user_id(message.from_user.id)
    await message.answer(get_sniper_status_text(), parse_mode="Markdown")


@dp.message()
async def free_symbol_analysis(message: Message) -> None:
    save_user_id(message.from_user.id)
    txt = (message.text or "").strip()
    if not txt or txt.startswith("/"):
        return
    sym = txt.split()[0].upper()
    if len(sym) > 12:
        await message.answer("Напиши короткий тикер, например BTC.")
        return
    wait = await message.answer(f"📊 Анализирую {sym}...")
    try:
        await wait.edit_text(await analyzer.full_analysis(sym), parse_mode="Markdown")
