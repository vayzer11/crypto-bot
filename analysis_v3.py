...
Источники: CoinGecko (бесплатно) + DeFiLlama (бесплатно) + Fear&Greed (бесплатно)
"""
...
class CryptoAnalyzer:
    ...
    async def _resolve_id(self, symbol: str) -> str:
        # Сначала проверяем наш список
        if symbol.upper() in SYMBOL_MAP:
            return SYMBOL_MAP[symbol.upper()]
        # Если нет — ищем через CoinGecko поиск
        try:
        ...
        try:
            ...
            if detail:
                ...
                if len(tvl_data) >= 2:
                    ...
                    if len(tvl_data) >= 7:
                        week_ago = tvl_data[-7].get("totalLiquidityUSD", 0)
    ...
    async def find_dump_risk(self) -> str:
        ...
        for r in risks[:8]:
            lines.append(
                f"💣 *{r['sym']}* — {fmt_price(r['price'])} | {fmt_b(r['cap'])}\n"
